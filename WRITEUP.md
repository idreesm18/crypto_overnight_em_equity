# Crypto Overnight Signal for Asian Equities — Pass 2 Writeup
...
Pass 1 confirmed this directional prediction. The gap-to-intraday IC ratio was roughly 3:1 in HK and 2.6:1 in KR, both with bootstrap p < 0.001. The SHAP top features loaded on crypto variables as expected. The net-of-cost backtest lost money on every tested configuration.

Pass 2 asks three narrower questions. First, is the observed predictability actually a crypto channel, or a short-term reversal effect inflated by the BTC-correlation-filtered universe? A parallel pipeline on a control universe with matched size and liquidity but no crypto-correlation filter isolates this. Second, does the signal survive transaction costs in any implementable form, once a borrow cost model and a regime gate are added? Third, does a temporal model on minute bars extract information the 21 hand-engineered features miss?

The answers in this sample: the crypto channel is partially identified in KR and not identified in HK; no single-stock strategy survives costs but two index-futures strategies do; the TCN is worse than LightGBM on gap IC.

## Data

Binance bulk archive, Stooq HK, pykrx (Oracle Cloud Seoul VM tunnel), FRED, and yfinance are reused from Pass 1 as documented there. Pass 2 adds the following sources.

yfinance index OHLCV for ^HSI (Hang Seng) and ^KS11 (KOSPI Composite) covers 2018-09-03 through 2026-04-17, 1,872 and 1,868 trading days respectively.

yfinance S&P 500 daily close (^GSPC) over the same window is the input to a market-cap proxy, computed as index level times a scaling factor calibrated to total S&P 500 market cap on 2024-12-31 (approximately $52.2T against a close of 5881.63, giving factor 8.875e9). BTC circulating supply is computed deterministically from the issuance schedule, anchored at 19.6M BTC on 2024-01-01, advancing at 900 BTC/day pre-halving and 450 BTC/day post the 2024-04-19 halving. The BTC-to-S&P 500 market-cap ratio, with its trailing 1-year median, is the regime gate variable.

Three Pass 1 dropped features were flagged for conditional restoration in Pass 2: log mcap bucket (KRX authenticated endpoint), BTC perp liquidation intensity (paid data), and USDT peg deviation (minute-level BTC/USD). None of the enabling credentials were available in this run. The feature set stays at Pass 1's 21 engineered features for stock universes and 18 for indices, with the restoration decision logged at `logs/feature_restoration_decisions.log`.

Two user-provided control CSVs at `data/stock_picks/crypto_control_{hk,kr}.csv` contain 42 HK and 39 KR ticker candidates. The columns `ticker, company_name, matched_sector, matched_treatment_example, rationale` document the manual curation. The control universes are drawn from liquid sectors with minimal crypto exposure (traditional banks, brokers, industrial conglomerates) and are matched on size and liquidity to the main universe.

## Methodology

### Universes

Pass 2 runs four universes in parallel.

*Main universe* is reused from Pass 1 without modification, 88 rebalance months from 2019-01-02 through 2026-04-01, median 24 HK and 30 KR tickers per month.

*Control universe* applies the same $500K ADV filter at each monthly rebalance, no BTC correlation filter, capped at 30 per market. 88 rebalance months HK, 87 KR, median pool 30 in each. Two tickers fell out of data (0273.HK Stooq file missing, 041140.KQ delisted). Control universes are logged at `output/control_universe_log.csv` and the features at `output/features_track_a_control_{hk,kr}.parquet`.

*KR KOSPI large-cap* restricts the KR main candidate list to .KS tickers with $50M trailing-20-day ADV, ranked by BTC correlation, top 12. Brief risk #3 materialized: only 14 of 39 KR candidates are .KS, and at the $50M threshold 83 of 88 rebalance months cannot form a pool of 9+ names. The variant is flagged inconclusive and downstream backtest configurations for large-cap are skipped. Detail at `logs/stage_p2-4_kospi_largecap.log`.
...
Track A is reused from Pass 1 for the main universe. The control and KOSPI large-cap universes use the same feature definitions, applied to their respective tickers. The index universe drops the three stock-level features and keeps the 18 crypto-plus-macro set. No lookahead.

Track B is the new raw-sequence track for the TCN. Per overnight window, minute bars for BTC, ETH, SOL are extracted, normalized per-channel per-window, and padded to the per-market maximum length with a boolean mask. 5-minute resampling was applied to keep the file size tractable (~1.7 GB total); 1-minute would have required ~57 GB. SOL pre-launch days (before 2020-08-11) are zero-filled for its five channels with the mask reflecting missing data. Static features (macro plus stock-level, or macro only for index) concatenate at the post-encoder stage. Files: `output/sequences_{hk,kr,index_hk,index_kr}.npz`.

### LightGBM

Walk-forward identical to Pass 1: expanding window, 252-day minimum training, monthly rebalance cadence, 3-fold purged time-series CV with 5-day purge, 10-iteration randomized hyperparameter search. Pass 2 added 12 LightGBM runs (6 control + 6 index). Pass 1's 6 main runs are reused, for 18 LightGBM models total. SHAP per fold is computed for new variants where feasible.
...
Training ran on Modal T4 GPUs. The runtime-adjustment ladder moved quickly: 5-minute sequences were used from the start, dropping dilation 16 and reducing max epochs to 15 (Step 1). Modal's default 2-hour per-function timeout was exceeded on the main-universe configs during the first attempt, because the longer main-universe sequence counts (40,808 HK rows, 51,876 KR rows) push the walk-forward to ~75 folds averaging 2.5 hours. The retry added per-fold checkpointing to the Modal volume, increased timeout to 4 hours, and used `starmap(return_exceptions=True)` so that a single-container failure does not cascade-cancel the siblings. The final run wrote 12 TCN prediction CSVs to local disk.

### Strategies

Stock universes use the Pass 1 tercile rule: long top third, short bottom third, equal weight within legs, one-day hold, flat if universe < 9 on a given day. A long-only variant is added: long top tercile, benchmarked against an equal-weight portfolio of that day's full universe. The long-only version carries no short leg and no borrow cost.
...
Half-spread per side: 15 bps HK main, 10 bps control HK and KR main, 5 bps control KR, 2 bps index (this last is an underestimate; see Limitations). Kyle-style impact `0.1 * sqrt(trade_size / ADV) * daily_vol` with $100K per position, applied to stock strategies only. Borrow cost annualized, applied daily to short notional: 500 bps HK main shorts, 300 bps control HK, 400 bps KR main, 250 bps control KR and KR KOSPI, 50 bps index. Long-only variants pay no borrow.
...
This is the central Pass 2 finding. The main universe's gap IC was interpreted in Pass 1 as evidence for a crypto channel. The control universe tests whether that interpretation survives the removal of the BTC-correlation filter.
...
In KR, the control gap IC is 0.024 lower than main with p=0.008, and the control gap-to-intraday ratio is 0.88 versus main's 2.15. The control ratio clears Rule A's upper bound of 1.5 but the main ratio misses Rule A's lower bound of 2.5. The direction is consistent with an incremental crypto channel that explains roughly 40 percent of the raw main gap IC, but magnitudes fall short of the strong-version acceptance threshold.
...
| Criterion | Pass 2 result | Verdict |
|---|---|---|
...
Subcategory tier: drop overnight returns has smaller effect than drop crypto-overnight-all, consistent with multiple crypto features carrying incremental information. Per-feature leave-one-out on 21 features across both markets: `stock_rv_20d` is the single most important feature (HK -0.92, KR -2.38 on net Sharpe), consistent with Pass 1's Stage 6 finding that the stock-level block is load-bearing first. Crypto overnight features are individually second-tier but jointly meaningful.

### Regime splits and long-only decomposition

Regime analysis on `output/regime_analysis_pass2.csv`: for index_kr gap, IC floors above 0.237 across VIX high/low, BTC up/down, and crypto-vol high/low cells; no regime cell inverts or collapses. For main universes, the KR 2023 attenuation from Pass 1 is not re-derived here by year but is known to concentrate in that window. `output/long_short_decomposition.csv` shows alpha concentration in the long leg for all four stock universes; the short leg adds gross signal but its cost profile (spread plus borrow) consumes the added contribution.
...
Three features originally specified in the brief remain dropped in Pass 2: log mcap bucket, BTC perp liquidation intensity, and USDT peg deviation. None of the enabling credentials were available in this run. The feature restoration framework is in place for a future run.
...
- `output/control_vs_main_comparison.csv` (the central Pass 2 result)
...
- `output/predictions_lgbm_{control,index}_{hk,kr}_{gap,intraday,cc}.csv` (12 new)
...
Review documents (twelve Pass 2 panelist papers plus synthesis):
...
Pass 1 decision logs (`logs/feature_decisions.log`) and Pass 2 restoration decisions (`logs/feature_restoration_decisions.log`) document each feature-level deviation from the brief.
