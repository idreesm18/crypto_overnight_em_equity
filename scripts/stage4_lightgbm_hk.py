# ABOUTME: Walk-forward LightGBM training for HK market, 3 targets: gap (PRIMARY), intraday, close-to-close.
# ABOUTME: Inputs: output/features_track_a_hk.parquet. Outputs: predictions, SHAP, training log CSVs/parquets per target.
# ABOUTME: Run: source .venv/bin/activate && python3 scripts/stage4_lightgbm_hk.py
...
INPUT_PATH = os.path.join(PROJECT_ROOT, "output", "features_track_a_hk.parquet")
...
def main():
    ...
    for target_name in TARGETS:
        ...
        # Save outputs
        pred_path = os.path.join(OUTPUT_DIR, f"predictions_lgbm_hk_{target_name}.csv")
        shap_path = os.path.join(OUTPUT_DIR, f"shap_per_fold_hk_{target_name}.parquet")
        log_path = os.path.join(OUTPUT_DIR, f"training_log_lgbm_hk_{target_name}.csv")
    ...
    print(f"\nOutput files:")
    for t in TARGETS:
        print(f"  output/predictions_lgbm_hk_{t}.csv")
        print(f"  output/shap_per_fold_hk_{t}.parquet")
        print(f"  output/training_log_lgbm_hk_{t}.csv")
